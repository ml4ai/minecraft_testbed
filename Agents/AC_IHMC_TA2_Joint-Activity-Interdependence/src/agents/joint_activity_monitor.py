...
class JointActivityMonitor(ASISTAgentHelper):
    ...
    def __init__(self):
        ...
        self.__topic_handlers = {
            ...
            'observations/events/player/marker_placed': self.__handle_marker_blocks
        }
    ...
    def __handle_proximity_block(self, header, message, data):
        ...
        self.get_logger().debug(f"proximity {poi_id}: player={player.id} {player.callsign} {action_type} {players_in_range}")
        those_in_proximity = []
        if poi_id in self.__proximity_map:
            those_in_proximity = self.__proximity_map[poi_id]
        if players_in_range == 2:
            for player_id in those_in_proximity:
                player = self.__players[player_id]
                rescue = player.joint_activity_model.get(aj.RESCUE_VICTIM['urn'], inputs)
                if rescue is not None:
                    unlock = rescue.get_by_urn(aj.UNLOCK_VICTIM['urn'], inputs)
                    if unlock is not None:
                        if not unlock.is_complete():
                            unlock.update_addressing(player_id, player_id, 0.0, elapsed_ms)
                            unlock.update_completion_status(player_id, True, elapsed_ms)
        if action_type == 'ENTERED_RANGE':
            those_in_proximity.append(player_id)
            self.__proximity_map[poi_id] = those_in_proximity
        elif action_type == 'LEFT_RANGE':
            those_in_proximity.remove(player_id)
            self.__proximity_map[poi_id] = those_in_proximity

    def __handle_marker_blocks(self, header, message, data):
        # print(f"PLACED: {data}\n")
        pass
    ...
    def compute_measure(self, jag: Jag):
        ...
        self.get_logger().debug(merged_jag.short_string())
        self.get_logger().debug(merged_jag.to_string())
