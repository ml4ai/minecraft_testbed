...
def update_knowledge(player_id, category, confidence_value, elapsed_ms):
    ...
    if player_id not in category:
        ...
        player_activity = category[player_id]
        last_tracker = player_activity.pop()
        if last_tracker.ongoing:  # handle ongoing activity
            ...
            if confidence_value > 0.0:  # only start tracking if confidence greater than zero
                category[player_id].append(ActivityTracker(player_id, confidence_value, TimePeriod(elapsed_ms, -1)))
                category[player_id] = get_non_overlapping_activity_tracker_set(category[player_id])
                return category[player_id][-1] != last_tracker
            else:
                return False
...
class Jag:
    ...
    def update_awareness(self, observer_player_id, aware_player_id, confidence_value, elapsed_ms):
        if self.is_leaf:
            if update_knowledge(aware_player_id, self.__awareness, confidence_value, elapsed_ms):
    ...
    def update_preparing(self, observer_player_id, preparing_player_id, confidence_value, elapsed_ms):
        if self.is_leaf:
            if update_knowledge(preparing_player_id, self.__preparing, confidence_value, elapsed_ms):
    ...
    def update_addressing(self, observer_player_id, addressing_player_id, confidence_value, elapsed_ms):
        if self.is_leaf:
            if update_knowledge(addressing_player_id, self.__addressing, confidence_value, elapsed_ms):
    ...
    # shallow is-active that does not check children used by jag_view to only color leaves
    def is_ongoing(self):
        for key in self.__addressing:
            for activity in self.__addressing[key]:
                if activity.ongoing:
    ...
    def short_string(self):
        ...
        elapsed_time = f" ({self.preparing_duration()/1000:.1f}sec: {self.addressing_duration()/1000:.1f}sec: {self.completion_duration()/1000:.1f}sec)"
        ...
        restricted = ''
        if self.__role_restriction is not None:
            restricted = '***'
        return f"{short_id}{inputs}{elapsed_time}{completion_status}{restricted}"
    ...
    def role_restriction(self):
        return self.__role_restriction

    def set_role_restriction(self, role):
        self.__role_restriction = role

    def is_role_restricted(self):
        return self.__role_restriction is not None

    def get_role_restriction(self):
        if self.is_role_restricted():
            return self.role_restriction

        if len(self.__children) == 0:
            return None

        if 'execution' in self.__connector.keys():
            execution = self.__connector['execution']
            if execution == 'node.execution.parallel':
                role_restriction = None
                for child in self.__children:
                    if not child.is_complete():
                        child_role_restriction = child.get_role_restriction()
                        if child_role_restriction is None:
                            return None
                        else:
                            role_restriction = child_role_restriction
                return role_restriction
            elif execution == 'node.execution.sequential':
                for child in self.__children:
                    if not child.is_complete():
                        role_restriction = child.get_role_restriction()
    ...
    def get_instance_data(self):
        ...
        if self.is_role_restricted():
            data['role-restricted'] = self.role_restriction
