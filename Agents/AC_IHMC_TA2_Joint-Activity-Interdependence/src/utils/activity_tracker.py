...
class ActivityTracker:
    ...
    def __init__(self, who, confidence=0.0, time_period=TimePeriod()):
        self.__who = who
    ...
    def dict(self):
        return {
            'participant_id': self.who,
            'confidence': self.confidence,
            'time_period': self.time_period.dict(),
        }
...
def get_activity_tracker_union(activity_tracker_1, activity_tracker_2):
    if not does_match(activity_tracker_1, activity_tracker_2):
        print("WARNING: activities do not match who or confidence and should: " + str(activity_tracker_1) + ": " + str(
            activity_tracker_2))
        ...
        while len(activity_tracker_set_copy) > 0:
            ...
            if len(non_overlapping_set) == 0:
                ...
                if does_time_period_overlap(activity_tracker, last_non_overlapping_activity_tracker):
                    merged_activity_tracker = get_activity_tracker_union(activity_tracker,
                                                                         last_non_overlapping_activity_tracker)
    ...
    if contains_overlapping_time_periods(activity_tracker_set):
        ...
        while len(activity_tracker_set_copy) > 0:
            ...
            if len(non_overlapping_set) == 0:
                ...
                last_non_overlapping_activity_tracker = non_overlapping_set.pop()
                if does_time_period_overlap(activity_tracker, last_non_overlapping_activity_tracker):
                    non_overlap_set = get_activity_tracker_non_overlap_union(activity_tracker,
                                                                             last_non_overlapping_activity_tracker)
