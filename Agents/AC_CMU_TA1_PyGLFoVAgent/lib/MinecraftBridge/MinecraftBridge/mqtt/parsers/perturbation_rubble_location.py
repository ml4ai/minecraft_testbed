# -*- coding: utf-8 -*-
"""
.. module:: perturbation_rubble_location
   :platform: Linux, Windows, OSX
   :synopsis: Parser for Perturbation Rubble Location messages

.. moduleauthor:: Dana Hughes <danahugh@andrew.cmu.edu>

Definition of a parser for Perturbation Rubble Location messages
"""

from ...messages import PerturbationRubbleLocations, Blockage
...
class PerturbationRubbleLocationsParser:
    """
    A class for parsing rubble perturbation messages from MQTT bus.
    ...
    topic = "observations/events/mission/perturbation_rubble_locations"
    MessageClass = PerturbationRubbleLocations

    msg_type = MessageType.event
    msg_subtype = MessageSubtype.Event_PerturbationRubbleLocations
    alternatives = []

    # Class-level logger
    logger = logging.getLogger("PerturbationRubbleLocations")
    ...
    def parse(cls, json_message):
        ...
        if busHeader.message_type != MessageType.event:
            return None
        if messageHeader.sub_type != MessageSubtype.Event_PerturbationRubbleLocations:
            return None
        ...
        message = PerturbationRubbleLocations(mission=mission)
