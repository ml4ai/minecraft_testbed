...
helpFunction()
{
	...
	echo -e "\t-z Do not update config paths"
...
while getopts "lzpjkfgnawdtrevucsmohbiq" opt
do
        case "$opt" in
                l ) do_run_lm="true";;
                z ) do_update_config_paths="false";;
...
echo "the value of do_update_config_paths after is: $do_update_config_paths"
...
if [[ $do_update_config_paths = "true" ]]; then
