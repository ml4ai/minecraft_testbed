...
def ac_cmu_ta1_pyglfovagent_test(name,lines,table:dict):
        ...
        try: 
            ...
            if ('topic' in json_line):
                ...
                if(topic == 'observations/events/mission'):
                    ...
                    elif( topic == 'agent/pygl_fov/player/3d/summary'):                                                       
                        num_fov_messages+=1
    ...
    if (num_fov_messages != 0):
        ...
        boolean1 = (elapsed_milliseconds/num_fov_messages <= (200) )
   
        out = ( elapsed_milliseconds/num_fov_messages )

    output[name+'_'+str(len(output.items()))] = name, str(boolean1) , '# avg ms/msg in mission : ' + str(int(out)) , 'max threshhold '+str(200)+' ms/msg'  
