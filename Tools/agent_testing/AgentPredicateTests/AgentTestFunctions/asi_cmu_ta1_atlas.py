...
def asi_cmu_ta1_atlas_test(name,lines,table:dict):
    ...
    num_chat_messages = 0
    num_chat_messages_two_and_a_half = 0
    two_and_a_half_min_in_milliseconds:int = (1000*60*2) + (30*1000)
            ...
            if ('topic' in json_line):
                ...
                if(topic == 'trial'):
                    ...
                    if(topic == 'observations/events/mission'):
                        ...
                        if( topic == 'agent/intervention/asi_cmu_ta1_atlas/chat'):
                            if(elapsed_milliseconds<=two_and_a_half_min_in_milliseconds):
    ...
    output[name+'_'+str(len(output.items()))] = name, str(boolean1) , '# chats in mission : ' + str(num_chat_messages) , '# messages > 0 if trial != Training'  
    output[name+'_'+str(len(output.items()))] = name, str(boolean2) , '# chats within 2.5 : ' + str(num_chat_messages_two_and_a_half) , "# messages > 0 within 2.5 minutes of mission start if trial != Training"   
