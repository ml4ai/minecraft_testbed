...
def ac_ihmc_ta2_player_proximity_test(name,lines,table:dict):
        ...
        try: 
            ...
            if ('topic' in json_line):
                ...
                if(topic == 'observations/events/mission'):
                    ...
                    elif( topic == 'observations/events/player/proximity'):                                                       
    ...
    if (num_proximity_messages != 0):
        boolean1 = (elapsed_milliseconds/num_proximity_messages <= 1000)
        out = (elapsed_milliseconds/num_proximity_messages)

    output[name+'_'+str(len(output.items()))] = name, str(boolean1) , '# avg time per msg : ' + str(int(out))+' ms' , 'avg msg time <= 1 per second'  
