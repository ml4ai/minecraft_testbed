...
def as_cmu_ta2_ted_test(name,lines,table:dict):
    ...
    for i in range(len(lines)):
        ...
        try: 
            ...
            if ('topic' in json_line):
                ...
                elif(topic == 'agent/ac/ac_cmu_ta2_ted/ted'):
                    if(mission_running):                        
                        ted_messages_during_mission+=1
    ...
    if(ted_messages_during_mission>=90):
        boolean1 = True
          
  

    output[name+'_'+str(len(output.items()))] = name, str(boolean1) , '# messages during mission : ' + str(ted_messages_during_mission) , '# messages >= 90 at the end of a 15min mission (~1 every 10sec)'  
