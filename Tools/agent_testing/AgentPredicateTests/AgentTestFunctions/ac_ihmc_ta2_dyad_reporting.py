...
def ac_ihmc_ta2_dyad_reporting_test(name,lines,table:dict):
    ...
    for i in range(len(lines)):
        ...
        try: 
            ...
            if ('topic' in json_line):
                ...
                if(topic == 'observations/events/player/dyad'):
                    num_dyad_messages+=1
    ...
    boolean1 = num_dyad_messages>=100

    #######################################################################################

    output[name+'_'+str(len(output.items()))] = name, str(boolean1) , '# dyad messages : ' + str(num_dyad_messages), "# messages >= 100 "   
