...
def ac_cmufms_ta2_cognitive_test(name,lines,table:dict):
    ...
    for i in range(len(lines)):
        ...
        try: 
            ...
            if ('topic' in json_line):
                ...
                if(topic == 'agent/measure/ac_cmufms_ta2_cognitive/load'):
                    num_load_messages+=1
    ...
    boolean1 = num_load_messages>=80

    #######################################################################################

    output[name+'_'+str(len(output.items()))] = name, str(boolean1) , '# load messages : ' + str(num_load_messages), "# load messages >= 80 "   
