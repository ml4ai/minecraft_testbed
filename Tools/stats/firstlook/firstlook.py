# Name: firstlook
# Description: Tool in the ASIST testbed that gathers basic statistics of a trial in elastic
#
...
def process_hits(hits, pub_cnt, nopub_cnt, message_type_cnts):
	global mqtt_client
	global cmd_args
...
def read_config_file(config_filespec):
	...
	cf["trial_query"] = config['ELASTIC']['trial_query']
	cf["use_ssl"] = config['ELASTIC'].getboolean('use_ssl')
	if "output_file" in config["ELASTIC"]:
		cf["output_file"] = config['ELASTIC']['output_file']
...
if cmd_args.trial_id != None:
	config["trial_query"] = fix_query(config["trial_query"],cmd_args.trial_id)
...
data = es.search(index=config["index"],  scroll='2m', size=config["size"], body=config["trial_query"])
...
while scroll_size > 0:
	...
	(pub_record_cnt, nopub_record_cnt, message_type_cnts) = process_hits(data['hits']['hits'],pub_record_cnt, nopub_record_cnt, message_type_cnts)
...
print('Total {cnt} records'.format(cnt=pub_record_cnt))
