...
DEFAULT_SUBSCRIPTIONS = {
    'trial': {'topic': 'trial', 'qos': 2},
    'agent/control/rollcall/request': {'topic': 'agent/control/rollcall/request', 'qos': 2},
}
...
class ASISTAgentHelper(object):
    ...
    def __init__(self, on_message_handler):
        ...
        self.__init_topic_subscriptions()
    ...
    def __init_topic_subscriptions(self):
        self.__subscriptions = {}

        # starts with registering default subscriptions
        for subscription in DEFAULT_SUBSCRIPTIONS.values():
            self.__init_topic_subscription(subscription)

        if "subscribes" not in self.__version_info.keys():
            return

        # registers configuration subscriptions, potentially overriding default subscriptions
        for subscription in self.__version_info['subscribes']:
            self.__init_topic_subscription(subscription)
            if 'qos' in subscription:
                del subscription['qos']

    def __init_topic_subscription(self, subscription):
            if 'topic' not in subscription:
                self.__logging.error(f"malformed subscription definition without topic property. {subscription}")
                return

            topic = subscription['topic']

            # appends the whole subscription definition to use its property when subscribing
            self.__subscriptions[topic] = dict(subscription)
    ...
    def subscribe(self, topic, message_type=None, sub_type=None, qos=DEFAULT_QOS):
    ...
    # Now subscribe to the topic if not already subscribed.
        if topic not in self.__subscriptions:
            self.__subscriptions[topic] = sub2
            if self.__connected_to_mqtt:
                self.__message_bus.subscribe(topic, qos)
    ...
    def unsubscribe(self, topic, message_type=None, sub_type=None):
        ...
        if unsubscribe and topic in self.__subscriptions and topic not in DEFAULT_SUBSCRIPTIONS:
    ...
    def send_msg(self, topic, message_type, sub_type, sub_type_version, timestamp=None, data=None, trial_key=None, msg_version="1.1", qos=0):
        ...
        self.send_message_generic(topic, json_dict, qos=qos)

    # Publish an MQTT message to the message bus with the given topic and JSON Dictionary
    def send_message_generic(self, topic, json_dict, qos=0):
        if topic is None or json_dict is None:
            return
        self.__message_bus.publish(RawMessage(topic, jsondata=json_dict), qos=qos)
    ...
    def __on_connection(self, is_connected, rc):
        ...
        if self.__connected_to_mqtt:
            ...
            for subscription in self.__subscriptions.values():
                topic = subscription['topic']
                qos = subscription['qos'] if 'qos' in subscription else DEFAULT_QOS
                self.__message_bus.subscribe(topic, qos)
                self.__logging.info(f"MQTT subscription: topic={topic}, qos={qos}, subscription={subscription}")
        else:
    ...
    def __publish_agent_version_message(self, trial_key):
        ...
        self.send_msg("agent/" + self.agent_name + "/versioninfo", "agent", "versioninfo", "0.1", trial_key=trial_key, data=self.__version_info, msg_version="0.6", qos=1)
    ...
    def __publish_rollcall_message(self, rollcall_id, trial_key):
        ...
        self.send_msg("agent/control/rollcall/response", "agent", "rollcall:response", "0.1", trial_key=trial_key, data=data, msg_version="0.6", qos=1)
